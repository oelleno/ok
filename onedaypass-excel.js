...
const fileName = "onedaypass.xlsx";

// docId 형식 확인 함수 - "YYMMDDone_NNN_NAME" 형식인지 확인
function isValidOnedayDocId(docId) {
...
export async function excelupload() {
  ...
  try {
    ...
    const dbInstance = await db;
    const docRef = doc(dbInstance, "Onedaypass", docId);
    ...
    const storageInstance = await storage;
    const fileRef = ref(storageInstance, `Onedaypass/${window.docId}/${fileName}`);
